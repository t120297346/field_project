...
image = cv2.imread('D:\\fruit_data\\raw_data\\fruit_train.tif')
labels = cv2.imread('D:\\fruit_data\\raw_data\\train_mask.tif')
...
for row in range(image.shape[0]):
    if gray[image.shape[0] - 1 - row, 0] != 255:
...
cv2.imwrite('D:\\fruit_data\\raw_data\\real_train.tif', image)
...
for i in range(x):
    for j in range(y):
        ...
        cv2.imwrite('D:\\fruit_data\\train\\%d.tif' %num, img)
        cv2.imwrite('D:\\fruit_data\\train_label\\%d.tif' %num, label)
        num += 1
